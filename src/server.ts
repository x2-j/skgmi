    ...
    client.guilds.cache
      ...
      ?.client.on('messageCreate', async (message) => {
        ...
        if (message.content === '!verify') {
          ...
          message.reply(
            "I've just sent you a verification link! Please check your DMs."
          );
          message.author.send(
            `Please login to EVE-Online via our verification portal: ${APP_URL}/${hashIt(message.author.id)}.\n\n Verification links are tied to Discord accounts and can be deleted at any time using \`!delete\` in the \`#eve-account-link\` channel.`
          );
        }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const character = await fetch(
        'https://esi.evetech.net/latest/characters/' + oauth.CharacterID + '/',
        authToken
      );
      ...
      discord?.guilds.cache
        ...
        .then((member) => {
          ...
          if (corp.name === 'Sakagami Incorporated')
            member.roles.add(
              '1256264080893804544',
              'Character is in Sakagami Incorporated'
            );
        });
